...
grid <- expand.grid(pi = seq(0.1,0.8,0.2), N=c(60,90,120), deltaA=0.8, deltaB=c(3),
                    pattern = c("spike"), k=c(0.5, 1, 2, 3),
  ...
  for (i in seq_len(nrow(grid))) {
    ...
    for(r in 1:Rreps){
      ...
      if (pattern == "monotone") {
        ...
        Y[Tt == 1 & B > 0] <- Y[Tt == 1 & B > 0] + deltaB * rk[Tt == 1 & B > 0] + sigmaB*Z
        Y[Tt == 1 & B==0] <- Y[Tt == 1 & B==0] + sigmaA*Z 
      }
      if (pattern == "mix"){
...
write.csv(grid, 'result/cor_spike.csv', row.names = F)
saveRDS(result.p, 'result/cor_spike_p.rds')
