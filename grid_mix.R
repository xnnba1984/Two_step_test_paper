...
grid <- expand.grid(pi = c(0,seq(0.1,0.8,0.1)), N=c(60,90,120), deltaA=c(0.6,0.8), 
                    deltaB=c(2,3),
                    pattern = c("mix"),
...
# write.csv(grid, 'result/mix_all.csv', row.names = F)
# saveRDS(result.p, 'result/mix_all.rds')

gg <- read.csv('result/mix_all.csv')
# grid <- rbind(grid,gg)
# r.p <- readRDS('result/mix_all.rds')
# result.p <- c(result.p, r.p)

g <- gg %>%                 # start from the long table
  mutate(
    deltaAB = factor(
      paste0("ΔA = ", deltaA, ",  ΔB = ", deltaB))
  )

heat_df <- g %>% 
  select(pi, N, deltaAB, AKSA = pAKSA, Fisher = pF, Brown = pCF) %>% 
  ...
  mutate(
    ...
    N      = as.numeric(N),
    deltaAB = factor(deltaAB, 
                    levels = sort(unique(deltaAB))),  # ensures row order
...
plot_df <- heat_df %>% 
  ...
  mutate(
    ...
    deltaB = factor(deltaAB, levels = sort(unique(deltaAB))),
    Method = factor(Method, levels = c("AKSA", "Fisher"))
  )

plot_df <- plot_df[plot_df$N!=30,]
plot_df <- plot_df[plot_df$deltaAB%in%c('ΔA = 0.6,  ΔB = 2', 'ΔA = 0.6,  ΔB = 3',
                                        'ΔA = 0.8,  ΔB = 2', 'ΔA = 0.8,  ΔB = 3'),]

ggplot(plot_df, aes(pi, Power, colour = Method, linetype = Method)) +
