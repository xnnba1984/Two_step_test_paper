...
grid <- expand.grid(pi = seq(0.1,0.8,0.2), N=c(60,90,120), deltaA=1, deltaB=c(2),
                    pattern = c("monotone"), k=c(0.5, 1, 2, 3),
...
#write.csv(grid, 'result/cor_mono.csv', row.names = F)
#saveRDS(result.p, 'result/cor_mono_p.rds')

grid <- read.csv('result/cor_mono.csv')
result.p <- readRDS('result/cor_mono_p.rds')
...
ggplot(summary_df, aes(x = cor_round, y = diff*100, group = 1)) +                               
  ...
  facet_grid(rows = vars(N), cols = vars(pattern),
             labeller = labeller(N  = \(x) paste("N =", x))) +   # 3 rows × 1 col
  labs(x = "\nSpearman correlation",
       y = "Power difference\n(Brown – Fisher)\n") +
  scale_x_continuous(breaks = sort(unique(summary_df$cor_round))) +
  scale_y_continuous(limits = c(-2,2), labels = number_format(accuracy = 1, suffix = "%")) +
...
grid.mono <- grid
grid.spike <- read.csv('result/cor_spike.csv')
grid.mix <- read.csv('result/cor_mix.csv')
grid.combine <- rbind(grid.mono, grid.spike, grid.mix)

summary_df <- grid.combine %>%                                     
  group_by(N, pattern, cor_round) %>%                             # 1) group by N and rounded ρ
  summarise(                                             # 2) average the power-gap
...
summary_df <- summary_df[summary_df$cor_round%in%c(.1,.2,.3,.4,.5),]
summary_df$pattern <- factor(
  tools::toTitleCase(summary_df$pattern),           # Capitalize first letter
  levels = c("Monotone", "Spike", "Mix")    # Desired display/order
)

ggplot(summary_df, aes(x = cor_round, y = diff, group = 1)) +                               
  geom_hline(yintercept = 0, linetype = "dashed") +    # diff = 0 reference
  geom_line(colour = "#E69F00", size = 1) +
  geom_point(colour = "#006D2C", size = 2.5) +
  facet_grid(rows = vars(N), cols = vars(pattern),
             labeller = labeller(N  = \(x) paste("N =", x))) +   # 3 rows × 1 col
  labs(x = '\nCorrelation',
       y = "Power difference\n(Brown – Fisher)\n") +
