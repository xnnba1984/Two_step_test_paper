...
grid <- expand.grid(pi = c(0,seq(0.1,0.8,0.1)), N=c(60,90,120), deltaA=1, deltaB=c(3),
                    pattern = c("monotone"), skew=c('mild','moderate','strong'),
...
write.csv(grid, 'result/mono_skew.csv', row.names = F)
saveRDS(result.p, 'result/mono_skew.rds')

# g <- read.csv('result/mono_all.csv')
# grid <- rbind(grid, g)
...
heat_df <- grid %>% 
  select(pi, N, deltaB, skew, AKSA = pAKSA, Fisher = pF, Brown = pCF) %>% 
