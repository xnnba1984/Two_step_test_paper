...
grid <- expand.grid(pi = c(0,seq(0.1,0.8,0.1)), N=c(60,90,120), deltaA=c(0.6), 
                    deltaB=c(2),
                    pattern = c("mix"), skew=c('mild','moderate','strong'),
...
g <- grid %>%                 # start from the long table
  mutate(
    deltaAB = factor(
...
heat_df <- g %>% 
  select(pi, N, deltaAB, skew, AKSA = pAKSA, Fisher = pF, Brown = pCF) %>% 
