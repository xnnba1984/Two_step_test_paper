...
grid <- expand.grid(pi = c(0,seq(0.1,0.8,0.1)), N=c(60,90,120), deltaA=c(0.8,1,1.2), deltaB=c(4),
                    pattern = c("spike"),
...
# write.csv(grid, 'result/spike_all.csv', row.names = F)
# saveRDS(result.p, 'result/spike_all.rds')
# 
grid <- read.csv('result/spike_all.csv')
# grid <- rbind(grid, g)
# r.p <- readRDS('result/spike_all.rds')
# result.p <- c(result.p, r.p)

heat_df <- grid %>% 
  select(pi, N, deltaA, AKSA = pAKSA, Fisher = pF, Brown = pCF) %>% 
...
ggplot(plot_df[plot_df$N!=30,], aes(pi, Power, colour = Method, linetype = Method)) +
  ...
  facet_grid(rows = vars(N),
             cols  = vars(deltaA),
             labeller = labeller(deltaA = \(x) paste0("Δ = ", x),
                                 N      = \(x) paste("N =", x))) +
