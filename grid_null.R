...
grid <- expand.grid(pi = seq(0,0.8,0.1), N=c(60,90,120), deltaA=1, deltaB=1,
                    pattern = c("null"), 
...
#write.csv(grid, 'result/type_I.csv', row.names = F)
#saveRDS(result.p, 'result/result_p_type_I.rds')

grid <- read.csv('result/type_I.csv')

# visualize type I control
df_long <- grid[grid$N!=30,] %>%     
  select(pi, N, pattern, AKSA = pAKSA, Fisher = pF, Brown = pCF) %>% 
...
#df_long <- df_long[df_long$Method!='AKSA',]

ggplot(df_long, aes(x = pi, y = err)) +
